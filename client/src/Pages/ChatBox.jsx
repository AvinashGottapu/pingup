...
      draw();
    }).catch(() => {
      // mic access denied — still show idle dots
    });
...
    const speechKey = import.meta.env.VITE_AZURE_SPEECH_KEY;
    const region = import.meta.env.VITE_AZURE_REGION;
    if (!speechKey || !region) { toast.error("Azure Speech credentials not configured"); return; }
...
        if (e.reason === SpeechSDK.CancellationReason.Error) toast.error(`Speech error: ${e.errorDetails}`);
...
        () => setIsListening(true),
        (err) => { toast.error(`Failed to start: ${err}`); setIsListening(false); }
      );
    } catch (err) {
...
          setIsListening(false);
        },
        (err) => { console.error("Stop error:", err); setIsListening(false); }
      );
    }
  };
...
      }
    };
    recognition.onerror = (e) => { if (e.error !== "no-speech") toast.error(`Mic error: ${e.error}`); };
...
  // Cancel: discard transcript, restore base text
  const cancelListening = () => {
    setText(baseTextRef.current);
    stopListening();
  };

  // Confirm: keep transcript, stop listening
...
            try {
              const token = await getToken();
              const { data } = await api.post("/api/message/call", {
                to_user_id: user._id,
                roomId,
                callerName: currentUser.full_name
              }, { headers: { Authorization: `Bearer ${token}` } });
              if (data.success) navigate(`/room/${roomId}`);
...
         className="flex-shrink-0 p-2 md:p-2.5 max-sm:mr-14 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white transition-all duration-200 hover:shadow-lg active:scale-95">
  ...
  {/* Show 18px icon on mobile, 20px on desktop */}
