...
const StoryViewer = ({viewStory,setViewStory}) => {   
   ...
   const renderContent = () => { 
       switch(viewStory.media_type)  { 
         ...
         case 'video' :  return ( 
            <video src={ viewStory.media_url } alt="" className='max-h-screen' onEnded={() => setViewStory(null)}
             controls autoPlay />
         )
         case 'text' :  return ( 
     ...
     style = {{backgroundColor : viewStory.media_type === 'text' ? viewStory.background_color : '#000000' }} >
       ...
       <div className='absolute top-0 left-0 w-full h-1 bg-gray-700'> 
        ...
        {/* User Info - Top Left */}
         ...
         <img src={viewStory.user?.profile_picture} alt="" className='size-7 sm:size-8 rounded-full object-cover
          border border-white'/> 
         ...
         {/* Close-button */} 
         <button onClick={handleClose} className=' absolute top-4 right-4 text-white text-3xl font-blod focus : outline-none'> 
            <X  className='w-8 h-5 hover:scale=110 transition cursor-pointer' />
         </button> 
