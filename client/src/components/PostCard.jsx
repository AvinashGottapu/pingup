...
import { BadgeCheck, Heart, MessageCircle, Share2, ThumbsUp, ThumbsDown, Trash2 } from "lucide-react";
...
const PostCard = ({ post }) => {
  ...
  try {
    ...
    const toxicityRes = await axios.post(
      `${AI_URL}/api/check-toxicity`,
      {
  ...
  const handleCommentLike = async (commentId) => {
    try {
      const { data } = await api.post('/api/post/comment/like', { postId: post._id, commentId }, {
        headers: { Authorization: `Bearer ${await getToken()}` }
      });
      if (data.success) {
  ...
  const handleCommentDislike = async (commentId) => {
    try {
      const { data } = await api.post('/api/post/comment/dislike', { postId: post._id, commentId }, {
        headers: { Authorization: `Bearer ${await getToken()}` }
      });
      if (data.success) {
        setComments(prev => prev.map(comment => comment._id === commentId ? {
    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow p-4 space-y-4 w-full max-w-2xl border dark:border-slate-800">
        ...
        >
          <Heart
            className={`w-4 h-4 cursor-pointer transition-colors ${
              isLiked ? 'text-red-500 fill-red-500' : 'text-gray-500 dark:text-gray-400 hover:text-red-400'
            }`}
          />
          <span className={isLiked ? 'text-red-500' : ''}>{likes.length}</span>
        </button>
        <button
      ...
      {showComments && (
        <div className="pt-3 border-t border-gray-200 dark:border-slate-700 space-y-4">
          ...
          {/* Write a comment input — fixed at top, never scrolls */}
          ...
          {/* Scrollable comments list — fixed 450px, hidden scrollbar */}
          {comments.length === 0 ? (
                ...
                <div key={index} className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-gray-400 dark:text-gray-500">
                      <button
                        className={`flex items-center gap-1 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors ${comment.likes_count?.includes(currentUser._id) ? 'text-indigo-600' : ''}`}
                        onClick={() => handleCommentLike(comment._id)}
                      >
                        <ThumbsUp className="w-3.5 h-3.5" />
                        <span className="text-xs">{comment.likes_count?.length || 0}</span>
                      </button>
                      <button
                        className={`flex items-center gap-1 hover:text-rose-600 dark:hover:text-rose-400 transition-colors ${comment.dislikes_count?.includes(currentUser._id) ? 'text-rose-600' : ''}`}
                        onClick={() => handleCommentDislike(comment._id)}
                      >
                        <ThumbsDown className="w-3.5 h-3.5" />
                        <span className="text-xs">{comment.dislikes_count?.length || 0}</span>
                      </button>
