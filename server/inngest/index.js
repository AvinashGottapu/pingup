...
const deleteStory = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        console.log(`[Inngest] Received story deletion event for Story: ${storyId}, User: ${userId}`);
        ...
        await step.run('delete-story', async () => {
            console.log(`[Inngest] Woke up. Attempting to delete story ${storyId} for user ${userId}`);
            const result = await Story.findOneAndUpdate(
                { user: userId },
                { $pull: { stories: { _id: new mongoose.Types.ObjectId(String(storyId)) } } },
                { new: true }
            )

            if (result) {
                const storyExists = result.stories.find(s => s._id.toString() === String(storyId));
                if (!storyExists) {
                    console.log(`[Inngest] Successfully deleted story ${storyId}`);
                } else {
                    console.log(`[Inngest] Failed to delete story ${storyId}. Story still in array.`);
                }
            } else {
                console.log(`[Inngest] User document not found for user ${userId}`);
            }
